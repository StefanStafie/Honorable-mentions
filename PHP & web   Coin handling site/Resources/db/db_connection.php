...
function OpenCon()
{
    ...
    //mysqli_real_connect($conn, "numaxremastered.mysql.database.azure.com", "Numax@numaxremastered", "ParolaAdmin1", "coins", 3306);
...
{
    $conn = OpenCon();
    $result = $conn->query("SELECT * FROM coins c join user_coins u on c.identifier = u.identifier where u.id_user = " .  $_SESSION['user_id'] . " " . $filter . " LIMIT " . $limit. " OFFSET " . $limit*($page-1));
    if (!$result)
        return false;
    return $result->fetch_all();
    CloseCon($conn);
...
{
    $conn = OpenCon();
    $result = $conn->query("SELECT count(*) FROM coins c join user_coins u on c.identifier = u.identifier where u.id_user = " .  $_SESSION['user_id'] . " " . $filter);
    if (!$result)
        return false;
    return $result->fetch_all();
    CloseCon($conn);
...
function addCoin($diameter, $weight, $axis, $collection, $coinUrl, $collUrl, $obverse, $identifier)
{
    $conn = OpenCon();
    $stmt = $conn->prepare('INSERT INTO coins (diameter, weight, axis, collection, object, col_uri, obv_ref, identifier) VALUES (?,?,?,?,?,?,?,?)');
    $stmt->bind_param('ssssssss', $diameter, $weight, $axis, $collection, $coinUrl, $collUrl, $obverse, $identifier);
    $worked = $stmt->execute();
    CloseCon($conn);
    return $worked;
}
...
function removeUserCoin($userId, $coinId) {
    $conn = OpenCon();
    $stmt = $conn->prepare("DELETE FROM user_coins WHERE id_user = ? AND identifier = ?");
    $stmt->bind_param("ss", $userId, $coinId); 
    $worked = $stmt->execute();
    CloseCon($conn);
    return $worked;
