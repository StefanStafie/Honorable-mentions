...
<body>
    ...
    <div id="red-canvas-orizontal">
        ...
        if (isset($_GET['identifier']) && $_GET['identifier'] != "")
            $filter .= " AND u.identifier like '%" . $_GET['identifier'] . "%'";
        ...
        $coins = getMyCoins($filter, $_GET['number'], $_GET['page']);
        if (!$coins) {
            echo "0 results";
        } else {
            echo "Showing items " . ($_GET['number'] * ($_GET['page'] - 1)) . " - " . ($_GET['number'] * ($_GET['page'] - 1) + (int) (count($coins))) . " of " . getMyCoinsTotalCount($filter)[0][0];
